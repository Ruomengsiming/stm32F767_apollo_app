...
u16 USART3_RX_STA = 0;													//USART3接收状态标记 bit15,接收完成标志 bit14,接收到0x0d(\n)

u8 USART1_aRxBuffer[USART1_RXBUFFERSIZE];									//HAL库使用的串口接收缓冲
u8 USART2_aRxBuffer[USART2_RXBUFFERSIZE];									//HAL库使用的串口接收缓冲
u8 USART3_aRxBuffer[USART3_RXBUFFERSIZE];									//HAL库使用的串口接收缓冲
...
void Uart3_Init(u32 bound)
{
	...
	/* 该函数会开启接收中断: 标志位UART_IT_RXNE, 并且设置接收缓冲以及接收缓冲接收最大数据量 */
	HAL_UART_Receive_IT(&UART3_Handler, (u8 *)USART3_aRxBuffer, USART3_RXBUFFERSIZE);
}
...
/**********************************************************************************************************
 @Function			void Uart3_PortSerialEnable(u8 xRxEnable, u8 xTxEnable)
 @Description			使能/失能 发送/接收 串口中断
 @Input				xRxEnable : 1 ENABLE
							  0 DISABLE
					xTxEnable : 1 ENBALE
 ...
 @Return				void
**********************************************************************************************************/
void Uart3_PortSerialEnable(u8 xRxEnable, u8 xTxEnable)
{
	ENTER_CRITICAL_SECTION();
	
	if (xRxEnable) {													//使能串口接收中断
		__HAL_USART_ENABLE_IT(&UART3_Handler, UART_IT_RXNE);
	}
	else {															//失能串口接收中断
		__HAL_USART_DISABLE_IT(&UART3_Handler, UART_IT_RXNE);
	}
	
	if (xTxEnable) {													//使能串口发送中断
		__HAL_USART_ENABLE_IT(&UART3_Handler, UART_IT_TXE);
	}
	else {															//失能串口发送中断
		__HAL_USART_DISABLE_IT(&UART3_Handler, UART_IT_TXE);
	}
