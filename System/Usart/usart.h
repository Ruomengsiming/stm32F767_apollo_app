...
extern u8 USART3_RX_BUF[USART3_REC_LEN];    									//USART3接收缓冲,最大USART3_REC_LEN个字节
...
extern u16 USART3_RX_STA;												//USART3接收状态标记 bit15,接收完成标志 bit14,接收到0x0d(\n)

extern u8 USART1_aRxBuffer[USART1_RXBUFFERSIZE];								//HAL库使用的串口接收缓冲
extern u8 USART2_aRxBuffer[USART2_RXBUFFERSIZE];								//HAL库使用的串口接收缓冲
extern u8 USART3_aRxBuffer[USART3_RXBUFFERSIZE];								//HAL库使用的串口接收缓冲
...
void Uart3_PortSerialEnable(u8 xRxEnable, u8 xTxEnable);						//使能/失能 发送/接收 串口中断
